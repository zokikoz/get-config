...
### Запуск
При запуске скрипта в архивной директории (по умолчанию **./archive**) создается каталог текущей даты. Если такой каталог уже есть, то создается новый с указанием метки времени. В случае если установлено несколько пулов устройств, в каталоге даты создаются подкаталоги с именами пулов.
...
Для запуска в тихом режиме (при использовании с cron) применяются аргументы ```-s``` или ```--silent```. Например:
```
./get-conf.rb --silent
```
